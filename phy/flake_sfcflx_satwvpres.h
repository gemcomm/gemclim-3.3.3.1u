...
REAL (KIND = ireals) FUNCTION flake_SfcFlx_satwvpres (T, h_ice)
...
IF(h_ice.LT.h_Ice_min_flk) THEN  ! Water surface
  flake_SfcFlx_satwvpres = b1_vap*EXP(b2w_vap*(T-b3_vap)/(T-b4w_vap))
ELSE                             ! Ice surface
  flake_SfcFlx_satwvpres = b1_vap*EXP(b2i_vap*(T-b3_vap)/(T-b4i_vap))
END IF 
...
END FUNCTION flake_SfcFlx_satwvpres
