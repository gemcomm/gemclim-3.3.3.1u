...
SUBROUTINE flake_SfcFlx_momsenlat ( height_u, height_tq, fetch,   &
...
REAL,    INTENT(IN) :: frac
INTEGER, INTENT(IN) :: trnch,i_trnch
...
INTEGER (KIND = iintegers), PARAMETER ::  &
  n_iter_max     = 36                       ! Maximum number of iterations 
...
wvpres_s = flake_SfcFlx_satwvpres(T_s, h_ice)  ! Saturation water vapour pressure at T=T_s
q_s = flake_SfcFlx_spechum (wvpres_s, P_a)     ! Saturation specific humidity at T=T_s
rho_a = flake_SfcFlx_rhoair(T_s, q_s, P_a)     ! Air density at T_s and q_s (surface values)
...
CALL flake_SfcFlx_roughness (fetch, U_a, u_star_min_sf, h_ice, c_z0u_fetch, u_star_thresh, z0u_sf, z0t_sf, z0q_sf)

!OPEN(UNIT=111, FILE='Roughness', POSITION='APPEND')
!WRITE(111,211)  z0u_sf, z0t_sf, z0q_sf, 1.0
...
CALL flake_SfcFlx_roughness (fetch, U_a, u_star_st, h_ice, c_z0u_fetch, u_star_thresh, z0u_sf, z0t_sf, z0q_sf)

!OPEN(UNIT=111, FILE='Roughness', POSITION='APPEND')
!WRITE(111,211) z0u_sf, z0t_sf, z0q_sf, 2.0
...
IF(U_a.LE.U_a_thresh) THEN  ! Smooth surface
  DO WHILE (Delta.GT.c_accur_sf.AND.n_iter.LT.n_iter_max) 
    CALL flake_SfcFlx_roughness (fetch, U_a, MIN(u_star_thresh, u_star_previter), h_ice,   &
                           c_z0u_fetch, u_star_thresh, z0u_sf, z0t_sf, z0q_sf)
!OPEN(UNIT=111, FILE='Roughness', POSITION='APPEND')
!WRITE(111,211) z0u_sf, z0t_sf, z0q_sf, 3.0
...
ELSE                        ! Rough surface
  DO WHILE (Delta.GT.c_accur_sf.AND.n_iter.LT.n_iter_max) 
    CALL flake_SfcFlx_roughness (fetch, U_a, MAX(u_star_thresh, u_star_previter), h_ice,   &
                           c_z0u_fetch, u_star_thresh, z0u_sf, z0t_sf, z0q_sf)

!OPEN(UNIT=111, FILE='Roughness', POSITION='APPEND')
!WRITE(111,211) z0u_sf, z0t_sf, z0q_sf, 4.0
...
!OPEN(UNIT=111, FILE='Roughness', POSITION='APPEND')
!OPEN(UNIT=111, FILE='Roughness')
!WRITE(111,211) z0u_sf, z0t_sf, z0q_sf, (c_Karman/LOG(height_u/z0u_sf))**2
...
!_dbg
END IF 
IF (z0t_sf<1.e-30_ireals .or. z0q_sf<1.e-30_ireals )           & ! Dugas B., 20110923
   write(*,'("lakefr,psi_q,psi_t,z0q_sf,z0t_sf=",2i5,5e15.5)') &
  trnch,i_trnch,frac,psi_q,psi_t,z0q_sf,z0t_sf
z0t_sf=max(1.e-30_ireals,z0t_sf) ; z0q_sf=max(1.e-30_ireals,z0q_sf)
...
211  FORMAT(4(G13.6, 1X))

END SUBROUTINE flake_SfcFlx_momsenlat
